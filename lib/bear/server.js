...
function isValidPostContentType(req) {
	var contentTypeHeaders = req.headers['content-type'] || '';
	contentTypeHeaders = contentTypeHeaders.split(';')[0];
	return contentTypeHeaders == 'application/json';
}

function isValidGetContentType(req) {
	return true; //hmm not sure about this....
}
	...
	Server.prototype[method] = function(path, schemaPath, fn) {
		...
		var routeHandler = function(req, res,next) {
			...
			if(!validateContentType(req)){
				...
				var requestValid = res.validateRequest(schemaPath, dataToValidate, function(e) {
					throw new Error(e);
				});
...
("get", function(req){return req.querystring;}, isValidGetContentType)("post", function(res) {return res.body;}, isValidPostContentType);
